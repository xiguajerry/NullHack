...
class DoubleSettingComponent<G : NullHackGui<G>>(
    x: Float, y: Float, width0: Float,
    private val _setting: DoubleSetting, father: ModuleComponent<G>
) : CommonSettingComponent<Double, G>(x, y, width0, _setting, father) {
    ...
    override fun render(mouseX: Int, mouseY: Int) {
        if (GLFW.glfwGetMouseButton(mc.window.window, GLFW.GLFW_MOUSE_BUTTON_1) != GLFW.GLFW_PRESS) dragging = false
        if (dragging) {
            ...
            while (true) {
                ...
                if (next < mouseHoldingOffset) {
                    nearestStop = next.toFloat()
                    current++
                    continue
                }
                if (mouseHoldingOffset in nearestStop..next.toFloat()) {
    ...
    override fun drawValue(mouseX: Int, mouseY: Int, fatherAlpha: Int) {
        val width = animationX.getAndUpdate(
            ...
            else width0 * getWidthMultiplier().toFloat()
        ) * (width / width0)
